...
**问题**:
用户报告记忆处理非常慢，前端长时间显示 "Loading memory..."：
```
[Mirix.TemporaryMessageAccumulator] INFO: Time taken to send to memory agents: 224.89 seconds
[Mirix.AgentWrapper] INFO: Time taken to absorb content into memory: 231.23 seconds
```

**根本原因**:
- 当前使用 **Meta Memory Agent 单线程模式** (`SKIP_META_MEMORY_MANAGER = False`)
- 1 个 LLM 需要分析所有截图（今天 157 张）
- LLM 串行调用，处理时间过长
- Gemini-2.5-flash API 响应慢

**性能分析**:

| 阶段 | 耗时 | 占比 |
|------|------|------|
| OCR 提取 + 存储 raw_memory | ~10s | 4% |
| Meta Memory Agent LLM 分析 | ~215s | 93% |
| 工具函数调用（插入记忆） | ~7s | 3% |
| **总计** | **~231s** | **100%** |

**优化方案**:
启用 **并行 Memory Agents 模式**：

```python
# mirix/agent/app_constants.py
SKIP_META_MEMORY_MANAGER = True  # 启用并行模式
```
...
| 模式 | 工作方式 | 预计耗时 | 优缺点 |
|------|---------|---------|--------|
| **Meta Memory Agent**<br>(旧) | 1个 LLM 决定所有记忆更新 | 🐌 **224s** | ✅ 更智能决策<br>❌ 非常慢 |
| **并行 Agents**<br>(新) | 6个 agents 并行处理 | ⚡ **40-60s** | ✅ 快 4-6 倍<br>❌ 可能冗余记忆 |

**并行模式原理**:
```python
# temporary_message_accumulator.py:889-906
with ThreadPoolExecutor(max_workers=6) as pool:
    futures = [
        pool.submit(
            send_to_agent,
            agent_type  # episodic, semantic, procedural, etc.
        )
...
**预期效果**:
- ✅ 处理时间: **224s → 40-60s** (减少 70-75%)
- ✅ 前端 Loading 时间大幅缩短
- ✅ 用户体验显著提升
- ⚠️  可能产生少量冗余记忆（6个 agents 独立决策）
...
# 等待下次记忆吸收，观察日志
# 预期: "Time taken to send to memory agents: 40-60 seconds"
```

**权衡考虑**:
1. **性能 vs 精确性**:
   - 旧模式: 1个 LLM 统一决策，更准确但慢
   - 新模式: 6个 LLM 独立决策，可能有冗余但快
...
3. **后续优化**:
   - 可以添加记忆去重逻辑
   - 实现增量处理（不一次处理 157 张截图）
   - 优化 LLM prompt 减少推理时间