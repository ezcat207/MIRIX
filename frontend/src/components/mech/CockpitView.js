...
const CockpitView = ({ activeMech, onCompleteTask, onMissionComplete }) => {
  ...
  // For now, we assume activeMech.tasks are just strings or simple objects
  // We'll just pick the first one for display
      ...
      <div className="hud-panel" style={{ gridRow: '2/3' }}>
        ...
        <div style={{ fontSize: '12px', color: '#aaa' }}>
          {activeMech?.tasks?.map((t, i) => (
            <div key={i} style={{ marginBottom: '5px', color: i === 0 ? '#fff' : '#aaa' }}>
      ...
      <div className="reticle-container">
        <div className="reticle">
          ...
          <div className="focus-task">
            {typeof currentTask === 'string' ? currentTask : currentTask.title || currentTask}
          </div>
