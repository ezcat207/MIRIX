"""
Simplified mock raw_memory data creation script.
Creates test data directly without querying for existing users.
...
try:
    ...
    if env_path.exists():
        ...
        if os.getenv('MIRIX_PG_URI'):
            print(f"✅ Using PostgreSQL: {os.getenv('MIRIX_PG_URI')}")
        else:
            print("⚠️  No MIRIX_PG_URI found, will use SQLite")
except ImportError:
...
def create_mock_data():
    ...
    test_org_id = "org-00000000-0000-4000-8000-000000000000"  # Use default org
    ...
    with db_context() as session:
        ...
        items = session.query(RawMemoryItem).filter(
            RawMemoryItem.user_id == test_user.id
        ).all()
        print(f"  Found {len(items)} raw_memory items in database for user {test_user.id}")
        if len(items) != 4:
            print(f"  ⚠️  Warning: Expected 4 items, found {len(items)}")

    print("\n🎯 Next Steps:")
    print("  1. Check database:")
    print(f"     python -c \"from mirix.server.server import db_context; from mirix.orm.raw_memory import RawMemoryItem; session = db_context(); items = session.query(RawMemoryItem).filter(RawMemoryItem.user_id == '{test_user.id}').all(); print(f'Found {{len(items)}} items'); [print(f'  {{i.source_app}}: {{i.source_url}}') for i in items]\"")
    print("\n  2. Start server and test frontend")
    print("     python -m mirix.server.fastapi_server")
    print("     # Ask: 'What did I work on with MIRIX?'")
